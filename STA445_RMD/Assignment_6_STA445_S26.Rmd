...
(Surv <- Survey %>% select(Year, Gender))
```
...
```{r}
(Surv <- Surv %>% mutate(Year = factor(Year, levels = c("FirstYear", "Sophomore", "Junior", "Senior"), ordered = TRUE)))
```

**d)** Convert the **Gender** column to factors and rename them Male/Female.
```{r}
Surv <- Surv %>% mutate(Gender = factor(as.character(Gender), levels = c("M", "F"), labels = c("Male", "Female")))
(unique(Surv$Gender))
```
...
```{r}
(datset <- Surv %>% drop_na() %>% count(Gender, Year))
```
...
```{r}
(datset2 <- datset %>% pivot_wider(names_from = Year, values_from = n, values_fill = 0))
```
...
```{r}
cbind(A, B)
inner_join( A, B, by = c("Name" = "First.Name")) %>% select(Name, Car, Pet)
...
cbind was easier to program but also likely to have an error especially with differening number of rows or clumns in the different data sets.
...
```{r}
dailytemps <- read_csv("https://raw.githubusercontent.com/BuscagliaR/STA_444_v2/master/data-raw/FlagMaxTemp.csv")
temps_long <- dailytemps %>% pivot_longer(cols = `1`:`31`, names_to = "Day", values_to = "MaxTemp")
temps_long <- temps_long %>% mutate(Day = as.numeric(Day), Date = as.Date(paste(Year, Month, Day, sep = "-")))
temps_long %>% filter(Year == 2005) %>% ggplot(aes(x = Date, y = MaxTemp)) + geom_line() + labs(title = "Max flag temps", x = "Date", y = "Max Temp")
```
...
```{r}
temps_long %>% filter(Year %in% c(2013, 2014, 2015)) %>% mutate(MonthDate = as.Date(paste(Year, Month, "01", sep = "-"))) %>%
  group_by(MonthDate) %>% summarize(AvgTemp = mean(MaxTemp, na.rm = TRUE)) %>% ggplot(aes(x = MonthDate, y = AvgTemp)) +
  geom_line() + labs(title = "2013 - 2015 flagstaff", x = "Date", y = "Avg Max Temp")
```
...
```{r}
feb14_flights <- flights %>% filter(month == 2, day == 14, carrier == "VX") %>% select(carrier, dest, dep_time, air_time)
(air_info <- left_join(feb14_flights, airports, by = c("dest" = "faa")))
```
...
```{r}
derekfilter <- Customers %>% filter(Name == "Derek Sonderegger")
(derek_statement <- derekfilter %>% inner_join(Cards, by = "PersonID") %>% inner_join(Transactions, by = "CardID") %>%
  inner_join(Retailers, by = "RetailID") %>% select(Name.x, DateTime, Amount, Retailer = Name.y))
```
...
Cards <- Cards %>% mutate(Exp_DateTime = ymd_hms(Exp_DateTime))
Cards <- Cards %>% mutate(Exp_DateTime = if_else(CardID == cardID, ymd_hms("2019-10-15 16:28:21"), Exp_DateTime))

new_card <- tibble(CardID = "1234567890123456", PersonID = personID, Issue_DateTime = ymd_hms("2019-10-15 16:28:21"),
  Exp_DateTime = ymd_hms("2022-10-15 16:28:21"))
Cards <- bind_rows(Cards, new_card)
```
...
```{r}
card <- Cards %>% filter(PersonID == aubrey$PersonID & Exp_DateTime > ymd_hms("2019-10-15 16:28:21")) %>% pull(CardID)
retailid <- 1 
datetime <- ymd_hms('2019-10-16 14:30:21')
amount <- 4.98
Valid_Cards <- Cards %>% filter(CardID == card, Issue_DateTime <= datetime, datetime <= Exp_DateTime)
...
card <- cardID 
retailid <- 3  
datetime <- ymd_hms('2019-10-17 10:00:00')
amount <- 100.00
...
```{r}
(aub_statement <- Customers %>% filter(Name == "Aubrey Sonderegger") %>% inner_join(Cards, by = "PersonID") %>%
  inner_join(Transactions, by = "CardID") %>% inner_join(Retailers, by = "RetailID") %>%
  select(Name.x, DateTime, Amount, Retailer = Name.y) %>% arrange(DateTime))
```
